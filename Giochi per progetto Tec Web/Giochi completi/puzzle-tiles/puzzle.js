...
(function(){
	...
	function createSquares() {
		...
		img.addEventListener("load", function() {
			...
			for( i = 0; i < numberOfSquares; i++ ) {
				...
				//this.i = int(random(0, squares.length));
				c.drawImage(
					img,
	...
	function random(min, max) {
	 ...
	 /**Questa funzione serve a valorizzare le due variabili, mouseX e mouseY, definite sopra. */
	puzzleCanvas.addEventListener('mousedown', function(e){
		mouseDown = true;
		mouseX = e.pageX - puzzleCanvas.offsetLeft;  // subtract the canvas offset for left and top, pageX corrisponde alla larghezza dell'elemento cliccato
			...
			if(mouseDown) {
				...
				if ( squares[i].drawX < mouseX && (squares[i].drawX + sqWidth) > mouseX && squares[i].drawY < mouseY && (squares[i].drawY + sqHeight) > mouseY ) {
					...
					if ( squares[i].drawX == spaceX && ( squares[i].drawY == spaceY - sqHeight || squares[i].drawY == spaceY + sqHeight ) || squares[i].drawY == spaceY && ( squares[i].drawX == spaceX - sqWidth || squares[i].drawX == spaceX + sqWidth ) ) {
						...
						c.clearRect(squares[i].drawX, squares[i].drawY, sqWidth, sqHeight);  // Clear the tile, i.e. create new space

						newSpaceX = squares[i].drawX;	// Store the position of this new space
						newSpaceY = squares[i].drawY;

						squares[i].drawX = spaceX;		// Store the position of the tile that will be drawn
